...
int main(int argc, char *argv[]) {
    ...
    while (fgets(r0, LINE_MAX, file)) {
        ...
        strcpy(r1, r0);
        cmdNum = 0;
        while ((cmds[cmdNum] = strtok(cmdNum == 0 ? r1 : NULL, "|")) != NULL) {
        ...
        for (int k = 0; k < cmdNum; k++) {
            ...
            while ((args[argNum] = strtok(argNum == 0 ? cmds[k] : NULL, " \n")) != NULL) {
                if (++argNum >= ARGS_MAX) FAILURE_EXIT(1, "Command %d exceeds maximum number (%d) of arguments", k,
                                                                   ARGS_MAX)
            }
            if (!argNum) continue;
            ...
            pid_t cp = fork();
            if (cp == -1) {
                FAILURE_EXIT(1, "Couldnt fork child process %d!\n", k);
            } else if (cp == 0) {
                if (k < cmdNum - 1) {
                    close(pipes[k % 2][0]);
