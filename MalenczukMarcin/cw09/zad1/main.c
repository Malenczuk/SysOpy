...
void sig_hnalder(int signo) {
    fprintf(stderr, "Recieved signal %s, Canceling threads\n", signo == SIGALRM ? "SIGALRM" : "SIGINT");
    for (int p = 0; p < P; p++)
...
void __init__() {
    if (nk == 0) signal(SIGINT, sig_hnalder);
    else signal(SIGALRM, sig_hnalder);
...
void *consumer(void *pVoid) {
    ...
    while (1) {
        ...
        if(length_search((int) strlen(line)))
            fprintf(stderr, "Consumer[%ld]: %d, %s",  pthread_self(), index, line);
        free(line);
    }
}
